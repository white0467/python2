...
max_pop = city_1nfo[0][1]     # 첫 번째 항목이 기준
min_pop = city_1nfo[0][1]        # 비교 기준
total_pop = 0   # 전체 변수 pop의 값을 초기화

max_city = city_1nfo[0][0]     # 첫 번째 항목이 기준
m1n_city = None     # 최소 인구를 가진 도시 초기화

#최대 인구를 가진 도시와 최소 인구를 가진 도시를 초기화.
# 도시 정보 리스트를 순회
for city, pop in city_1nfo:      
    # 전체 인구 합계를 계산
    total_pop += pop
...
print('최대인구: {}, 인구: {} 천명'.format(max_city, max_pop))
print('최소인구: {}, 인구: {} 천명'.format(m1n_city, m1n_pop))
